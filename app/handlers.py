...
def sparkline(values: list[Decimal]) -> str:
    ...
    return "".join(
        bars[min(7, int((value - low) / (high - low) * 7))]
        for value in values
    )
...
async def build_prices_screen() -> tuple[str, InlineKeyboardMarkup]:
    async with SessionLocal() as session:
        rows = (
            await session.execute(
                select(Drug, MarketPrice, MarketState)
                .join(MarketPrice)
                .outerjoin(MarketState)
            )
        ).all()
    ...
    for drug, price, state in rows:
        ...
        lines.append(
            ...
            f"{eur(current)} {arrow}{abs(change):.1f}%"
        )

        buttons.append(
            InlineKeyboardButton(
                text=drug.display_name,
                callback_data=f"market:{drug.code}",
            )
        )
...
async def build_market_detail(code: str) -> tuple[str, InlineKeyboardMarkup]:
    ...
    async with SessionLocal() as session:
        ...
        history = list(
            (
                await session.scalars(
                    select(MarketHistory.price)
                    .where(
                        MarketHistory.drug_id == drug.id,
                        MarketHistory.recorded_at >= cutoff,
                    )
                    .order_by(MarketHistory.recorded_at.asc())
                )
            ).all()
        )
        ...
        volume = int(
            (
                await session.scalar(
                    select(func.coalesce(func.sum(TradeRecord.quantity), 0))
                    .where(
                        TradeRecord.drug_id == drug.id,
        ...
        news = (
            await session.execute(
                select(MarketNews)
                .where(MarketNews.drug_id == drug.id)
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🛒 Buy",
                    callback_data=f"buy:{drug.code}",
                ),
                InlineKeyboardButton(
                    text="💰 Sell",
                    callback_data=f"sell:{drug.code}",
                ),
            ],
            [
                InlineKeyboardButton(
                    text="📊 Back to market",
                    callback_data="nav:prices",
                ),
                InlineKeyboardButton(
                    text="🏠 Main menu",
...
async def build_raid_screen(
    user_id: int,
) -> tuple[str, InlineKeyboardMarkup, timedelta]:
    ...
    async with SessionLocal() as session:
        ...
        if cooldown > timedelta(0):
            text = (
                ...
                f"You can raid again in: "
                f"<b>{countdown_text(cooldown, False)}</b>"
            )
        ...
        rows = [
            [
                InlineKeyboardButton(
                    text=(
                        f"🎯 {target.username or target.display_name} "
                        f"({eur(target.money)})"
                    ),
...
def admin_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="📅 Minutes per day per player",
                    callback_data="admin:daily_player",
                )
            ],
            [
                InlineKeyboardButton(
                    text="📊 Total minutes per day",
                    callback_data="admin:daily_total",
                )
            ],
            [
                InlineKeyboardButton(
                    text="👥 Total minutes per player",
                    callback_data="admin:player_total",
                )
            ],
            [
                InlineKeyboardButton(
                    text="Σ Grand total",
                    callback_data="admin:grand",
                )
            ],
            [
                InlineKeyboardButton(
...
# ---------------------------------------------------------
# Railway / Telegram diagnostic
# ---------------------------------------------------------

@router.message(Command("ping"))
async def ping(message: Message) -> None:
    print(
        f"PING received from Telegram user {message.from_user.id}",
        flush=True,
    )
    await message.answer("🏓 Pong! Railway bot is responding.")
        ...
        if action == "home":
            ...
            inventory = (
                await session.execute(
                    ...
                    .join(Drug)
                    .where(
                        Inventory.user_id == player.user_id,
            ...
            lines = [
                "💵 <b>Wallet</b>",
                "",
                f"Cash: {eur(player.money)}",
                f"Tokens: 🔑 {player.tokens}",
                "",
                "📦 Inventory:",
            ]

            lines.extend(
                f"• {drug.display_name}: {item.quantity}"
                for item, drug in inventory
            )
            ...
            rows = (
                await session.execute(
                    select(Drug, MarketPrice).join(MarketPrice)
                )
            ).all()

            text = (
                "🛒 <b>What would you like to buy?</b>"
                if action == "buy"
                else "💰 <b>What would you like to sell?</b>"
            )
            ...
            for drug, price in rows:
                if action == "buy":
                    ...
                    label = (
                        f"{drug.display_name} — owned: {owned}"
                    )
                    shown_price = price.current_price
            ...
            text, keyboard, cooldown = await build_raid_screen(
                player.user_id
            )
            ...
            news_rows = (
                await session.execute(
                    select(MarketNews, Drug)
                    .outerjoin(
                        Drug,
                        Drug.id == MarketNews.drug_id,
                    )
                    .order_by(
            ...
            if news_rows:
                for item, drug in news_rows:
                    impact = (
                        f" ({Decimal(item.impact_percent):+.0f}%)"
                        if Decimal(item.impact_percent)
                        else ""
                    )
            ...
            rows, total = await player_playtime(
                session,
                player.user_id,
            )
            ...
            lines = [
                "⏱ <b>My playtime</b>",
                "",
                f"Total: <b>{duration_text(total)}</b>",
                "",
                "Recent days:",
            ]

            lines.extend(
                f"• {row.play_date.isoformat()}: "
                f"{duration_text(row.seconds_played)}"
                for row in recent
            )
            ...
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="💰 Money Boost — 5 🔑",
                            callback_data="boost:MONEY_BOOST",
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text="🛡️ Raid Shield — 3 🔑",
                            callback_data="boost:RAID_SHIELD",
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text="📈 Price Boost — 4 🔑",
                            callback_data="boost:PRICE_BOOST",
                        )
                    ],
                    [
                        InlineKeyboardButton(
            ...
            lines = [
                "🗝 <b>Black Market</b>",
                "",
                f"Your keys: 🔑 <b>{player.tokens}</b>",
                "",
                (
                    "Keys are earned from daily logins, "
                    "level-ups, trades and successful raids."
                ),
            ]
            ...
            text = (
                ...
                + "\n".join(
                    (
                        f"{rank}. "
                        f"{escape(item.username or item.display_name)} "
                        f"— {eur(item.money)}"
                    )
...
async def admin_reports(callback: CallbackQuery) -> None:
    ...
    if action == "daily_player":
        lines = [
            "📅 <b>Minutes per day per player</b>",
            "",
        ]
        ...
        for row, player in report["per_day_player"]:
            lines.append(
                f"{row.play_date.isoformat()} · "
                f"{escape(player.username or player.display_name)}: "
                f"{row.seconds_played // 60}m"
            )

    elif action == "daily_total":
        lines = [
            "📊 <b>Total minutes of all players per day</b>",
            "",
        ]
        ...
        lines.extend(
            f"{day.isoformat()}: "
            f"{int(seconds) // 60}m"
            for day, seconds in report["per_day_totals"]
        )

    elif action == "player_total":
        lines = [
            "👥 <b>Total minutes per player</b>",
            "",
        ]
        ...
        lines.extend(
            ...
            for player, seconds in report["per_player_totals"]
        )

    else:
        lines = [
            "Σ <b>Total playtime of all players</b>",
            "",
            f"{report['grand_total'] // 60} minutes",
            f"({duration_text(report['grand_total'])})",
        ]
...
async def live_market_detail(
    message,
    code: str,
) -> None:
    try:
        while True:
            text, keyboard = await build_market_detail(
                code
            )
...
@router.callback_query(
    F.data.startswith(("buy:", "sell:"))
)
async def choose_trade_quantity(
    ...
    rows.append(
        [
            InlineKeyboardButton(
                ...
                callback_data=f"custom:{action}:{code}",
            )
        ]
    )
    ...
    if action == "sell" and owned > 0:
        rows.append(
            [
                InlineKeyboardButton(
                    text=f"Sell all ({owned})",
                    callback_data=(
                        f"trade:sell:{code}:{owned}"
                    ),
                )
            ]
        )
    ...
    info = (
        f"Current price: "
        f"{eur(price.current_price)} each\n"
        f"You own: <b>{owned} units</b>"
    )

    await callback.message.edit_text(
        (
            f"<b>{drug.display_name}</b>\n"
            f"{info}\n\nChoose a quantity:"
        ),
...
async def request_custom_quantity(
    callback: CallbackQuery,
    state: FSMContext,
) -> None:
    ...
    await callback.message.edit_text(
        "✍️ Send the number of units as a message.\n\n"
...
@router.message(
    Command("cancel"),
    StateFilter(TradeInput.waiting_quantity),
)
async def cancel_custom_trade(
    message: Message,
    state: FSMContext,
) -> None:
...
async def execute_custom_trade(
    message: Message,
    state: FSMContext,
) -> None:
    ...
    if quantity < 1 or quantity > 10000:
        await message.answer(
            "Enter a quantity between 1 and 10,000."
        )
        return
    ...
    if action == "buy" and quantity > 100:
        await message.answer(
            "A single purchase is limited to 100 units."
        )
        return
    ...
    async with SessionLocal() as session:
        ...
        try:
            ...
            token_found = await maybe_trade_token_drop(
                session,
                player,
            )

            level, gained = await grant_xp(
                session,
                player,
                quantity,
            )
    ...
    await message.answer(
        (
            f"✅ {verb} {quantity} units "
            f"for {eur(amount)}.{extra}"
        ),
        reply_markup=main_menu(),
    )
...
async def execute_trade(
    ...
    _, action, code, quantity = (
        callback.data.split(":")
    )

    async with SessionLocal() as session:
        if not await mark_callback(
            session,
            callback.id,
            callback.from_user.id,
            callback.data,
        ...
        try:
            ...
            message = (
                f"✅ "
                f"{'Purchased' if action == 'buy' else 'Sold'} "
                f"{quantity}× {code.title()} "
                f"for {eur(amount)}.{extra}"
            )
...
async def create_loan(
    ...
    async with SessionLocal() as session:
        ...
        try:
            ...
            message = (
                f"✅ Loan received: "
                f"{eur(loan.principal)}. "
                f"Repayment: "
                f"{eur(loan.total_due)}."
            )
...
async def black_market_purchase(
    callback: CallbackQuery,
) -> None:
    ...
    async with SessionLocal() as session:
        ...
        try:
            name = await buy_black_market_item(
                session,
                player,
                code,
            )
    ...
    await callback.message.edit_text(
        f"✅ Purchased: <b>{escape(name)}</b>",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="🗝 Back to Black Market",
                        callback_data="nav:blackmarket",
                    )
                ],
                [
                    InlineKeyboardButton(
